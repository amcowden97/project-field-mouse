...
DEFAULT_EMPTY_RETENTION_DAYS = 1
DEFAULT_DETECTION_RETENTION_DAYS = 3
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        help="Station TOML; PFM_CONFIG is used when omitted.",
    )
...
def main() -> int:
    ...
    if arguments.empty_retention_days is None:
        arguments.empty_retention_days = (
            config.storage.empty_recording_retention_days
        )
    if arguments.detection_retention_days is None:
        arguments.detection_retention_days = (
            config.storage.detection_recording_retention_days
        )
    rare_species = [
